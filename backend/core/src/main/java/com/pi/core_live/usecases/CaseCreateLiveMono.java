...
@Service
public class CaseCreateLiveMono implements Callable<Mono<Live>>, ITokenCheck {
    private static final Logger LOG = LoggerFactory.getLogger(CaseCreateLiveMono.class);

    private ILiveCommandCacheOut liveCommandOut;
    private JwtDecoder jwtDecoder;
    private CommandDto dto;

    public CaseCreateLiveMono() {}
    ...
    /**
     * Sets the CommandDto that will be used to create a live.
     *
     * @param dto the CommandDto to use for creating a live.
    ...
    /**
     * Executes the use case to create a live.
     *
    ...
    public Mono<Live> call() throws GlobalException {
        LOG.info("Init CaseCreateLiveMono call.");
        dto.validate();
        checkCredentials(jwtDecoder, dto.token(), dto.login(), dto.code());
        var response = liveCommandOut.createLive(dto.token(), dto.login(), dto.code(), dto.keyQuiz());
        LOG.info("End CaseCreateLiveMono call.");
        return response;
    }
}
