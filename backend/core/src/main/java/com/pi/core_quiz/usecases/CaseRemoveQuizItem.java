...
/**
 * Service class for handling the removal of quiz items.
 * This class implements the {@link Callable} interface to remove a quiz item
 * based on the provided command DTO.
 */
@Service
public class CaseRemoveQuizItem implements Callable<Response> {
    private static final Logger LOG = LoggerFactory.getLogger(CaseRemoveQuizItem.class);
    ...
    /**
     * Executes the remove quiz item operation.
     *
    ...
    public Response call() throws GlobalException {
        ...
        dto.validate();
        var response = quizCommandOut.deleteQuizItem(dto.key(), dto.position());
